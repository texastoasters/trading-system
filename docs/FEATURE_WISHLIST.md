...
- [x] **Executor: sell fill race condition** — `execute_sell` assumes fills are immediate. Fixed: `stop_cancelled` flag + stop-loss restore in exception handler. PR #57.
- [x] **Executor: accept qty=0 orders** — Executor now rejects `quantity <= 0` at top of both `execute_buy` and `execute_sell`. PR #57.
- [x] **Watcher/PM feedback loop on qty=0 positions** — PM dedup check (`evaluate_entry_signal` line 112) already rejects entry when position exists in Redis, including stale qty=0 entries. Verified by tests in `test_portfolio_manager.py`. PR #58.
- [x] **PM: qty=0 order after DOWNTREND halving** — DOWNTREND halved position size *after* the `< 1 share` guard → `int(0.5) = 0` published to executor. Fixed: guard added after halving step. PR #58.
- [x] **Executor: submits equity market orders after market close** — `clock.is_open` check added to both `execute_buy` and `execute_sell`. PR #57.
...
- [ ] **`scripts/reconcile.py`** — Compare Redis positions vs Alpaca actual positions. Identify missing stop-losses, phantom positions, mismatched quantities. Run on startup and daily. Explicitly called out as needed in HANDOFF.md. **Next priority.**
...
- [x] **Correlated regime adjustment** — DOWNTREND halves position sizes for equity. Fixed edge case where halving produced 0 shares (PR #58).
...
1. ~~Fix the 5 known bugs (HANDOFF.md)~~ ✅ **All done** (PRs #57, #58)
...
*Last updated: 2026-04-08. All critical bugs resolved. Coverage: config.py, indicators.py, notify.py, executor.py all at 100% (141 tests). portfolio_manager.py partial coverage started.*
