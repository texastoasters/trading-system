defmodule DashboardWeb.DashboardLiveTest do
  ...
  describe "equity_history accumulation" do
    defp equity_state(equity), do: %{"trading:simulated_equity" => equity}

    test "equity_tick starts at 0", %{conn: conn} do
      {:ok, view, _} = live(conn, "/")
      assigns = :sys.get_state(view.pid).socket.assigns
      assert assigns.equity_tick == 0
    end

    test "equity_history starts empty", %{conn: conn} do
      {:ok, view, _} = live(conn, "/")
      assigns = :sys.get_state(view.pid).socket.assigns
      assert assigns.equity_history == []
    end

    test "equity_tick increments on each state_update", %{conn: conn} do
      {:ok, view, _} = live(conn, "/")
      send(view.pid, {:state_update, equity_state(5000.0)})
      render(view)
      assigns = :sys.get_state(view.pid).socket.assigns
      assert assigns.equity_tick == 1
    end

    test "equity_history stays empty before 15 updates", %{conn: conn} do
      {:ok, view, _} = live(conn, "/")
      for _ <- 1..14, do: send(view.pid, {:state_update, equity_state(5000.0)})
      render(view)
      assigns = :sys.get_state(view.pid).socket.assigns
      assert assigns.equity_history == []
    end

    test "equity_history records point at tick 15", %{conn: conn} do
      {:ok, view, _} = live(conn, "/")
      for _ <- 1..15, do: send(view.pid, {:state_update, equity_state(4900.0)})
      render(view)
      assigns = :sys.get_state(view.pid).socket.assigns
      assert assigns.equity_history == [4900.0]
    end

    test "equity_history records second point at tick 30", %{conn: conn} do
      {:ok, view, _} = live(conn, "/")
      for _ <- 1..15, do: send(view.pid, {:state_update, equity_state(4900.0)})
      for _ <- 1..15, do: send(view.pid, {:state_update, equity_state(4950.0)})
      render(view)
      assigns = :sys.get_state(view.pid).socket.assigns
      assert assigns.equity_history == [4950.0, 4900.0]
    end

    test "equity_history ignores update when equity is absent from state", %{conn: conn} do
      {:ok, view, _} = live(conn, "/")
      for _ <- 1..15, do: send(view.pid, {:state_update, %{}})
      render(view)
      assigns = :sys.get_state(view.pid).socket.assigns
      assert assigns.equity_history == []
    end

    test "renders intraday equity sparkline section", %{conn: conn} do
      {:ok, _view, html} = live(conn, "/")
