...
### Fixed
- **Stop-loss qty drift → repeating "Stop-loss failed" critical alert + lost P&L.** When a GTC stop partially filled (e.g. 2 of 3 shares) and was then cancelled, nothing reconciled the sold shares: Redis `quantity` stayed at 3 while Alpaca held 1, and the 2 sold shares' realized P&L was never booked to simulated equity or the trades table. Every watcher exit cycle then ran `execute_sell` → cancel stop → market-sell qty 3 → `403 insufficient qty (available: 1)` → restore-stop qty 3 → 403 again → `critical_alert("Stop-loss failed …")`, looping indefinitely (observed live on TTE). The fix has two parts:
  - **Idempotent delta booking (`_book_partial_stop_fill`)** — whenever Redis qty exceeds the broker's actual qty and the position's stop order shows `filled_qty > 0`, the unbooked delta (`redis_qty − broker_qty`) is booked **once** at the stop's `filled_avg_price`: realized P&L hits simulated equity, a `stop_loss_partial` trade is logged, and Redis qty is synced to the broker. It's self-idempotent — after booking, `redis == broker`, so a second call from another path books nothing (no double-counting). It only books when the stop actually filled shares, never fabricating a fill for a position closed by another route.
  - **Broker-truth reconcile on both paths** — `_alpaca_position_qty()` reads the broker's real position size. `execute_sell` books the partial then syncs qty (cleaning Redis + clearing `exit_signaled` if nothing remains) before selling; `_check_cancelled_stops` does the same before resubmitting a stop. Unknown/transient reads (API error or non-list response) fall back to the Redis qty so a blip never deletes a live position.

### Tests
- 24 new cases in `skills/executor/test_executor.py`: `_alpaca_position_qty` (present/absent/non-list/api-error/abs/non-numeric-type/bad-string); `_book_partial_stop_fill` (no-fill, unparseable qty, zero-delta idempotency, books delta + syncs qty + correct −$8.44 P&L, fill-price fallbacks for bad-string and non-numeric, crypto fee); `execute_sell` reconcile (clamp + sell remaining, books the 2 prior-stop shares @ 83.87, prior-stop-read error still clamps, position-gone cleanup + `exit_signaled` clear, api-error fallback, matching-qty no-op); `_check_cancelled_stops` (partial-fill book + resubmit remainder, full-fill-then-gone cleanup, no-partial broker clamp).
- `skills/executor/executor.py` at **100%** coverage; full suite 1101 pass.

### Ops
- The live stuck position self-heals on the next post-deploy exit cycle: whichever path runs first books the 2 shares' realized P&L (≈ −$8.44 on TTE: 83.87 vs 88.09 entry), syncs Redis to the broker's 1 share, and either sells it or resubmits a 1-share stop — no double-booking. No manual server intervention. Note `scripts/reconcile.py --fix` still does **not** address this (it resubmits stops at the stale Redis qty); folding the broker-qty sync + delta booking into `reconcile.py` is a worthwhile follow-up.
